...
test.describe("Header Component", () => {
  test("Basic header", async ({ page }) => {
    ...
    await expect(page.locator("#storybook-root")).toHaveScreenshot("header-basic.png");
  });

  test("With labels", async ({ page }) => {
    await page.goto("/iframe.html?id=components-organisms-header--with-labels&viewMode=story");
    await page.waitForLoadState("networkidle");
    await expect(page.locator("#storybook-root")).toHaveScreenshot("header-with-labels.png");
  });
});
