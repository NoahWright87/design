...
- **Automated npm publish (new):** `.github/workflows/publish.yml` publishes to npm on every push to `main` where `package.json`'s version is ahead of what's live on the registry; fails loudly instead of publishing if it's ever behind. Authenticates via npm Trusted Publishing (OIDC) — no `NPM_TOKEN` secret involved, since npm is deprecating 2FA-bypass tokens for CI publishing. Requires a one-time "Trusted Publisher" registration for this workflow on the package's npmjs.com settings.
- **PR preview publish (new):** `.github/workflows/publish-pr-preview.yml` publishes an ephemeral pre-release build (`<version>-pr<N>.<sha>`) under the `pr-<N>` npm dist-tag on every push to a pull request, and comments the install command (`npm install @noahwright/design@pr-<N>`) so changes can be tried before merging. Never touches `package.json` in the repo. Also uses Trusted Publishing, registered as its own entry separate from `publish.yml`.
