...
- **Automated npm publish (new):** `.github/workflows/publish.yml` publishes to npm on every push to `main` where `package.json`'s version is ahead of what's live on the registry; fails loudly instead of publishing if it's ever behind. Requires an `NPM_TOKEN` repo secret.
