...
- **Hero organism + TextCarousel molecule (new):** `Hero` lays out a title/media row (media floats beside the title at every screen width, shrinking considerably on narrow ones rather than stacking below) with tagline/description/actions stacked full-width underneath — the intro-banner pattern factored out of a consuming site into a reusable organism. Title/tagline render exactly as given (no imposed heading level or styling), matching how `Header`/`Footer` slots already work, so a call site's own `Heading` keeps its `gradient`/`animateIn`/etc. Gained `background` (`subtle`/`primary`/`secondary` tint, with matching padding) and `bottomBorder` (`solid` divider or primary→secondary `gradient` bar) options for visually distinguishing a hero section. `TextCarousel` rotates a list of strings with a choice of animation (`crossfade`, `sequential`, `typewriter`); the typewriter engine and reduced-motion detection are their own exported atoms (`useTypewriter`, `usePrefersReducedMotion`) so either composes outside `TextCarousel` too. `Carousel` gained `showControls`/`decorative` props so it can double as Hero's ambient, controls-free rotating-photo media. The portfolio example site's hero section now runs on `Hero` in place of its bespoke markup.
- **Hero/TextCarousel polish:** Added the Storybook stories that were missing for both (neither showed up in the side nav despite being implemented). Fixed a reduced-motion blank gap in `TextCarousel`'s sequential mode, made `Carousel`'s `decorative` flag always force `showControls` off (a focusable control inside an `aria-hidden` region is a real a11y bug), and cleaned up a misleading alt text / CSS sizing conflict in the portfolio example. `useTypewriter` now randomizes typing speed and dwell duration around their averages (deleting stays constant, simulating a held backspace key) and adds a brief pause before typing the next word, so the effect reads as human rather than metronomic. Sequential fade and typewriter both now reserve layout space for the widest/tallest item up front, so they never shift surrounding content when items wrap to a different number of lines. Typewriter transitions are now diff-based (new `typewriterDiff` atom): a word shared between the outgoing and incoming phrase, anywhere in it, stays put instead of being cleared and retyped, and a changed word sharing a prefix with its replacement (e.g. "build" -> "builder") is edited only past that shared prefix — so "Software engineer" -> "Software builder" only touches the second word. Prefix matching and slicing are Unicode-code-point-aware (not raw UTF-16 code units), so two emoji sharing a surrogate-pair's high half are never misread as sharing a character and split into a broken glyph.
- **Hero: media aligned with title:** Media now floats beside the title specifically (in its own `.nw-hero__top` row) rather than centered against the whole tagline/description/actions column, at both wide and narrow screens — narrow screens only shrink media's width (220px → 80px), they no longer switch it to stack below. Retargeted the portfolio example's custom hero CSS to the new `.nw-hero__top`/`.nw-hero__title` class names.
- **Typewriter transitions rebuilt to mimic real editing:** `typewriterDiff` now resolves changes the way a person actually edits a sentence — starting from the caret's current position, it works backward, fully resolving each change (clearing and writing a run of wholly different words, or correcting a similar word in place) as it's reached, gliding rather than jumping through everything already correct in between, with nothing deferred to a later pass. Word-pair matching is now gated by actual Levenshtein similarity (`similarityThreshold`, not just tied alignment cost), so two unrelated words sharing one incidental letter no longer get paired. `useTypewriter`/`TextCarousel` gained a distinct, slower/more-jittery timing tier (`editTypingSpeed`/`editTypingSpeedJitter`/`editDeletingSpeed`/`editPauseBeforeTyping`) for in-place corrections versus fresh words, plus a `moveSpeed` for the caret's glide.
