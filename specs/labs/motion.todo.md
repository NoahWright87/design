...
### UI & Interaction
- [x] Groupable preset categories (e.g., "Professional", "Playful", "Extreme").
- [x] Keyboard shortcuts for preset selection (Ctrl+1, Ctrl+2, etc.).
- [ ] Real-time animation playhead on easing graphs (shows progress along curve).
- [ ] Undo/redo stack for knob changes.
- [x] "Reset to preset" button in addition to preset selector.
...
### Output Formats
- [x] Export as SCSS variables (with !default).
- [x] Export as CSS custom property definitions (copy into `:root`).
- [ ] Export as Tailwind animation config (animated utility).
- [x] Export as design token JSON (for token studio integration).
...
- [x] Animation speed multiplier (0.5x–2x playback speed).

### Accessibility
- [ ] Focus visible state for easing graph (interactive elements).
- [ ] Keyboard-draggable bezier control points (instead of sliders).
- [x] Screen reader descriptions for easing curve shapes (e.g., "overshoot at top").
